...
Prior to adding a feature, we would research the feature, the libraries required and the most sensible approach.
However, sometimes documentation for this would be sparse or difficult to understand.
Here, an LLM, primarily ChatGPT, Claude, or Gemini was used to find further information about a topic.
Once a feature had been thoroughly researched, if we had not quite found a good approach to implementation, an LLM was also used to sketch an outline for the approach.
This usually worked well, as LLMs are good at reading large amounts of documentation, extracting the important parts, and relaying them to you.
We did not consider this as co-authorship, since no code from an LLM was used, neither verbatim nor as inspiration.

When we did co-author an LLM, the commit was annotated with a `Co-authored-by: ChatGPT <chatgpt@openai.com>` footer in the git commit.
...
This had some success, however, often the code was not quite right, or plainly didn't work, which can be seen in
